...
/**
 * A helper class for {@link androidx.paging.PagedList.BoundaryCallback MovieBoundaryCallback}s and
 ...
 * <pre>
 * class PagingBoundaryCallback extends PagedList.MovieBoundaryCallback&lt;MyItem> {
 ...
 * <pre>
 * MutableLiveData&lt;PagingRequestHelper.NetworkState> combined = new MutableLiveData&lt;>();
 * helper.addListener(status -> {
 *     // merge multiple states per request type into one, or dispatch separately depending on
 *     // your application logic.
 *     if (status.hasRunning()) {
 *         combined.postValue(PagingRequestHelper.NetworkState.RUNNING);
 *     } else if (status.hasError()) {
 *         // can also obtain the error via {@link StatusReport#getErrorFor(RequestType)}
 *         combined.postValue(PagingRequestHelper.NetworkState.FAILED);
 *     } else {
 *         combined.postValue(PagingRequestHelper.NetworkState.SUCCESS);
 *     }
 * });
 * </pre>
 */
...
public class PagingRequestHelper {
    ...
    public enum RequestType {
         ...
         * a {@link androidx.paging.PagedList.BoundaryCallback MovieBoundaryCallback}.
         */
        INITIAL,
        /**
         * Corresponds to the {@code loadBefore} calls in {@link DataSource} or
         * {@code onItemAtFrontLoaded} in
         * {@link androidx.paging.PagedList.BoundaryCallback MovieBoundaryCallback}.
         */
        BEFORE,
        /**
    ...
    public static final class StatusReport {
        /**
         * NetworkState of the latest request that were submitted with {@link RequestType#INITIAL}.
         */
        @NonNull
        public final Status initial;
        /**
         * NetworkState of the latest request that were submitted with {@link RequestType#BEFORE}.
         */
        @NonNull
        public final Status before;
        /**
         * NetworkState of the latest request that were submitted with {@link RequestType#AFTER}.
         */
        @NonNull
